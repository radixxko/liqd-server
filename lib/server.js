...
const isMethods = ( methods ) => ( typeof methods === 'string' && METHODS.includes( methods ) || ( Array.isArray( methods ) && !methods.find( m => !METHODS.includes( m ))));
const isMiddlewares = ( middlewares ) => ( typeof middlewares === 'function' || ( Array.isArray( middlewares ) && middlewares.find( m => typeof m === 'function' )));
				...
				client:
				{
					accept: async( request, socket ) =>
					{
						if( !this._options.client.accept || await this._options.client.accept( request, socket ))
						{
							...
							if( this._routes.WS.find( r => this._router[r].routes.find( r => r.match( uri ))) !== undefined )
							{
								return true;
							}
						}
